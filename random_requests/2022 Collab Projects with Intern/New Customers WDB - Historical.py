...
###ADD DYNAMIC BRANCH NAME AND BRANCH ID IF YOURE ACTUALLY GOING TO USE THIS

# %sql
# CREATE TABLE IF NOT EXISTS power_bi_data_products.retail_new_customers_3 (utc_timestamp DATE, customer_key BIGINT, new_customer_flag BIGINT, returning_customer_flag BIGINT, lost_customer_flag BIGINT, relost_customer_flag BIGINT, most_recent_acquisition_date DATE, dynamic_branch_name STRING, dynamic_branch_id BIGINT)
# USING DELTA
...
date_df = date_df.where(date_df.utc_timestamp>=date(2022,5,5))
...
for index, item in enumerate(date_list):
    if index == 0:
        prior_item = date(2022,5,4) # prior file date
        ...
        new_cust = new_cust.select('utc_timestamp','customer_key','new_customer_flag','most_recent_acquisition_date')
        ...
        returning_cust = returning_cust.select('utc_timestamp','customer_key','returning_customer_flag','most_recent_acquisition_date')
...
prior_item = date(2022,4,29) # prior file date
        
todays_date = date(2022,5,2) # acquisition dates
prior = prior_item # loss date
