...
# read in rm_master to get processing dates
...
date_df = date_df.where(date_df.utc_timestamp >= date(2022, 5, 5)) # this should be the processing date before the missed dates
...
print(date_list) # check that this is all missing dates plus the file prior to them
...
for index, item in enumerate(date_list):
    if index == 0:
        ...
        spark.sql("""
        merge into customer_acquisition.acquisition_dates sink
        ...
        df = spark.table('customer_acquisition.acquisition_dates')
