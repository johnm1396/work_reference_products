...
# MAGIC LOCATION "/mnt/empriselaboratory/development/John/dynamic_branch_assignments"
...
# MAGIC %sql
# MAGIC CREATE TABLE IF NOT EXISTS dynamic_branch_assignments.dynamic_branch_assignments (customer_key INT, primary_branch STRING, direct_branch STRING, acct_branch STRING, opening_branch STRING, dynamic_branch_assignment STRING, is_active STRING, is_direct STRING, utc_timestamp DATE)
# MAGIC USING DELTA
...
# read in *hidden* (new teller transaction journal)
*hidden* = spark.read.option("header", True).option("inferSchema", True).parquet("/mnt/*hidden*/*/*/*/*.parquet")

# filter out irrelevant transactions and drop any duplicates for the most recent version
*hidden* = *hidden*.where(*hidden*.primary_owner_key > 0)
*hidden* = *hidden*.where(*hidden*.account_key_1.isNotNull())
*hidden* = *hidden*.orderBy('utc_timestamp', ascending=False).dropDuplicates(['transaction_id'])

# drop irrelevant columns
*hidden* = *hidden*.select('account_key_1', 'branch_number', 'transaction_date', 'transaction_id')

# union classic and *hidden* transactions
teller_union = teller_trans.union(*hidden*)
