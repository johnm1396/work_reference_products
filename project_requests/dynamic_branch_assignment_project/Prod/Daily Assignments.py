...
cust_data = cust_data.select('customer_key', 'opening_branch').dropDuplicates()
...
primary = primary.groupBy(['customer_key', 'branch_number']).count()
...
# MAGIC %md
# MAGIC DYNAMIC ASSIGNMENTS TO DELTA TABLE
...
branch_assignments = branch_assignments.withColumnRenamed('primary_branch_number', 'primary_branch_id')
branch_assignments = branch_assignments.withColumnRenamed('direct_branch_number', 'direct_branch_id')
branch_assignments = branch_assignments.withColumnRenamed('acct_branch_number', 'acct_branch_id')
branch_assignments = branch_assignments.withColumnRenamed('opening_branch', 'rm_branch_id')
branch_assignments = branch_assignments.withColumnRenamed('dynamic_branch_assignment', 'dynamic_branch_id')

branch_assignments = branch_assignments.withColumn('primary_branch_name', branch_assignments.primary_branch_id)
branch_assignments = branch_assignments.withColumn('direct_branch_name', branch_assignments.direct_branch_id)
branch_assignments = branch_assignments.withColumn('acct_branch_name', branch_assignments.acct_branch_id)
branch_assignments = branch_assignments.withColumn('rm_branch_name', branch_assignments.rm_branch_id)
branch_assignments = branch_assignments.withColumn('dynamic_branch_name', branch_assignments.dynamic_branch_id)

branch_assignments = branch_assignments.select('customer_key', 'primary_branch_id', 'primary_branch_name', 'direct_branch_id', 'direct_branch_name', 'acct_branch_id', 'acct_branch_name', 'rm_branch_id', 'rm_branch_name', 'dynamic_branch_id', 'dynamic_branch_name', 'is_active', 'is_direct', 'utc_timestamp')
...
branch_assignments = branch_assignments.withColumn('primary_branch_id', branch_assignments.primary_branch_id.cast('string'))
branch_assignments = branch_assignments.withColumn('primary_branch_name', branch_assignments.primary_branch_name.cast('string'))
branch_assignments = branch_assignments.withColumn('direct_branch_id', branch_assignments.direct_branch_id.cast('string'))
branch_assignments = branch_assignments.withColumn('direct_branch_name', branch_assignments.direct_branch_name.cast('string'))
branch_assignments = branch_assignments.withColumn('acct_branch_id', branch_assignments.acct_branch_id.cast('string'))
branch_assignments = branch_assignments.withColumn('acct_branch_name', branch_assignments.acct_branch_name.cast('string'))
branch_assignments = branch_assignments.withColumn('rm_branch_id', branch_assignments.rm_branch_id.cast('string'))
branch_assignments = branch_assignments.withColumn('rm_branch_name', branch_assignments.rm_branch_name.cast('string'))
branch_assignments = branch_assignments.withColumn('dynamic_branch_id', branch_assignments.dynamic_branch_id.cast('string'))
branch_assignments = branch_assignments.withColumn('dynamic_branch_name', branch_assignments.dynamic_branch_name.cast('string'))

# convert branch numbers to names
branch_lookup = spark.read.option("header", True).option("inferSchema", True).parquet(f"/mnt/*hidden*/{today}/*.parquet").orderBy('utc_timestamp', ascending=False).select('branch_id', 'branch_description').dropDuplicates(['branch_id'])
...
branch_assignments = branch_assignments.na.replace(dict, subset=['primary_branch_name', 'direct_branch_name', 'acct_branch_name', 'rm_branch_name', 'dynamic_branch_name'])

# write to delta table
branch_assignments.createOrReplaceTempView('branch_assignments')

merge_record = spark.sql("""
merge into dynamic_branch_assignments.dynamic_branch_assignments sink
using branch_assignments source
on source.customer_key = sink.customer_key and source.utc_timestamp = sink.utc_timestamp
when not matched then insert *""")
...
counts = spark.table('dynamic_branch_assignments.dynamic_branch_assignments')
display(counts.groupBy('utc_timestamp').count().orderBy('utc_timestamp', ascending=False))
